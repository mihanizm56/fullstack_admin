...
export const deleteUser = async (req, res) => {
  const { id } = req.params;
  const sanitizedUserId = sanitize(id);
  console.log("check id of user to delete", sanitizedUserId);

  try {
    await deleteUserByIdFromDb({ sanitizedUserId });
    await deleteNewByUserId({ userId: sanitizedUserId });
